...
def main():
    ...
    parser.add_argument('--apiKey', default=os.environ.get('RANCHER_ACCESS_KEY'),
                        help='api key, $RANCHER_API_KEY environment variable can be used')
    parser.add_argument('--apiSecret', default=os.environ.get('RANCHER_SECRET_KEY'),
    ...
    required_named.add_argument('--action',
                                help='add-link,  remove-lnk, create-stack, remove-stack, get-port, get-service-port, upgrade-service, get-container-id, get-host-ip, update-labels, get-service, confirm-upgrade')
    ...
    if args.action.lower() not in ['add-link', 'remove-link', 'create-stack',
                                   ...
                                   'get-svc-id', 'get-container-id',
                                   'get-host-ip', 'update-labels', 'get-service',
                                   'confirm-upgrade']:
        parser.parse_args(['-h'])
        exit(2)
    ...
    if args.action.lower() == 'get-port':
        ...
        service.update_load_balancer_service(
            args.loadBalancerId, data)

    elif args.action.lower() == 'update-labels':
        service.update_labels(args.host, data)
    ...
    elif args.action.lower() == 'get-service':
        service.get(args.host)

    elif args.action.lower() == 'confirm-upgrade':
        service.confirm_upgrade(args.host)
