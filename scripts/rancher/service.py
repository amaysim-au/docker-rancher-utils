...
def get_service(service_id):
    ...
    end_point = '{}/projects/{}/services/{}'.format(api.V2_BETA, project_id, service_id)
...
def update_labels(host_name, labels_data=None):
    """Update service labels"""
    service_id = parse_service_id(host_name)
    data = __get(service_id)

    data_upgrade = data['upgrade']
    labels = data_upgrade['inServiceStrategy']['launchConfig']['labels']
    for key in labels_data:
        labels[key] = labels_data[key]

    data_upgrade['inServiceStrategy']['launchConfig']['labels'] = labels

    __init_upgrade(service_id, data_upgrade)
    __wait_for_upgrade(service_id)
    __wait_for_healthy(service_id)
    __finish_upgrade(service_id)
